...
def perfect_square(score):
    return True if sqrt(score).is_integer() else False

def next_perfect_square(score):
    if perfect_square(score) == True:
        root = int(sqrt(score)) + 1
        return root**2
# END PROBLEM 4
...
def max_scoring_num_rolls(dice=six_sided, total_samples=1000):
    ...
    s = 0
    for i in range(1,11):
        thisAvg = make_averaged(roll_dice,total_samples)
        res = thisAvg(i,dice)
        if res > m:
            s = i
            m = res
...
def tail_strategy(score, opponent_score, threshold=12, num_rolls=6):
    ...
    """
    # BEGIN PROBLEM 10
    return num_rolls if tail_points(opponent_score) < threshold else 0 # Remove this line once implemented.
    # END PROBLEM 10
...
def square_strategy(score, opponent_score, threshold=12, num_rolls=6):
    ...
    if perfect_square(score + tail_point):
        temp = next_perfect_square(score + tail_point)
        if temp - score >= threshold:
            return 0
    elif perfect_square(score + tail_point) == False:
